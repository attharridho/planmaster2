document.addEventListener('DOMContentLoaded', () => {
    ...
    // Menggunakan LocalStorage sebagai database pengganti
    ...
    function initApp() {
        // Load data dari penyimpanan lokal browser
    ...
    // Expose function to window so HTML onclick works
    window.saveTask = function() {
        ...
        const id = idInput.value || Date.now().toString(); // Generate ID unik jika baru
        ...
        // Collect uploaded files with CONTENT
    ...
    // --- 6. TASK LIST LOGIC (WITH CATEGORY FILTER) ---
    ...
    window.handleFileSelect = function(files) {
        Array.from(files).forEach(file => {
            if(file.size > 500 * 1024) { 
                showNotification('File terlalu besar (>500KB) untuk penyimpanan lokal!', true);
                const fileData = { name: file.name, size: (file.size/1024).toFixed(1) + ' KB', type: file.type, content: null };
                renderFileItem(fileData, true);
                setTimeout(() => {
                    const item = document.querySelector(`div[data-name="${file.name}"]`);
                    if(item) {
    ...
    function renderFileItem(file, isUploading = false) {
        ...
        if(file.content) {
            div._fileContent = file.content;
            div.dataset.content = file.content;
        }

        const isImage = file.type.includes('image');
        const iconClass = isImage ? 'fa-image text-purple-500' : 
                          file.type.includes('pdf') ? 'fa-file-pdf text-red-500' : 'fa-file text-gray-500';

        let iconHtml = `<i class="fas ${iconClass} text-lg"></i>`;
        if(isImage && file.content) {
            iconHtml = `<img src="${file.content}" style="width: 32px; height: 32px; object-fit: cover;" class="rounded border border-gray-300">`;
        }

        let viewBtn = '';
        if(file.content) {
            viewBtn = `<a href="${file.content}" target="_blank" class="text-blue-500 hover:text-blue-700 mr-2" title="Lihat"><i class="fas fa-eye"></i></a>`;
        }
        ...
        div.innerHTML = `
            <div class="flex items-center gap-3 overflow-hidden">
                ...
                <div class="flex flex-col overflow-hidden">
                    <span class="truncate font-medium dark:text-gray-200">${file.name}</span>
                    <span class="text-xs text-gray-500">${file.size}</span>
                </div>
            </div>
            ${isUploading ? `
            <div class="loading-state w-24">
                <div class="h-1 w-full bg-gray-200 rounded overflow-hidden">
                    <div class="progress-bar h-full bg-blue-500 w-0 transition-all duration-1000"></div>
                </div>
            </div>` : ''}
    ...
    function updateTimerDisplay() {
        ...
        if(circle) {
            if(isTimerRunning) circle.classList.add('timer-active');
            else circle.classList.remove('timer-active');
        }
    }
    ...
    window.startTimer = function() {
        ...
        timerInterval = setInterval(() => {
            if (timeLeft > 0) {
                ...
                new Notification("Waktu Habis!", { body: "Istirahat sejenak." });
                showNotification("Waktu Habis! Ambil istirahat.");
                window.resetTimer();
            }
        }, 1000);
    };
    ...
    function updateStats() {
        ...
        for(let i=6; i>=0; i--) {
            ...
            const count = tasks.filter(t => {
                ...
                if (t.completedAt) return t.completedAt === dateStr;
    ...
    function setupEventListeners() {
        ...
        // PERBAIKAN: Tombol Hamburger Menu (3 Strip) dengan logika Collapsed
        ...
        if(menuToggle && sidebar) {
            menuToggle.addEventListener('click', (e) => {
                e.stopPropagation();
                // Cek apakah layar Desktop (lebar > 768px)
                const isDesktop = window.innerWidth >= 768;
                
                if (isDesktop) {
                    // Logic Desktop: Toggle kelas 'collapsed' (width: 0) agar kalender melebar
                    sidebar.classList.toggle('collapsed');
                } else {
                    // Logic Mobile: Toggle class '-translate-x-full' (Overlay)
                    sidebar.classList.toggle('-translate-x-full');
                    
                    // Tambahkan Z-Index agar muncul paling atas di mobile
            ...
            // Tutup sidebar jika klik di luar (Fitur UX Mobile)
            document.addEventListener('click', (e) => {
                const isDesktop = window.innerWidth >= 768;
                const clickedOutside = !sidebar.contains(e.target) && !menuToggle.contains(e.target);
                
                // Hanya tutup otomatis di mobile jika sidebar terbuka
                if (!isDesktop && !sidebar.classList.contains('-translate-x-full') && clickedOutside) {
    ...
    function showNotification(msg, isError = false) {
        const div = document.createElement('div');
        div.className = `fixed bottom-4 right-4 ${isError ? 'bg-red-600' : 'bg-gray-800'} text-white px-4 py-2 rounded shadow-lg z-50 text-sm animate-bounce`;
        div.textContent = msg;
